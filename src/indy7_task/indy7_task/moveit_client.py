...
주요 흐름:
  ...
  5. 필요 시 multi-seed IK 후보와 trajectory safety check로 후보를 거른다
...
class Indy7MoveItClient:
    ...
    COST_WEIGHTS = [1.0, 1.2, 1.5, 1.0, 1.3, 6.0]
    ...
    def wait_for_servers(self, timeout_sec=10.0):
        ...
        if not self.ik_client.wait_for_service(timeout_sec=timeout_sec):
            self.node.get_logger().error("compute_ik 서비스를 사용할 수 없습니다")
            return False
        return True
    ...
    def _send_request(self, request, plan_only=False):
        ...
        if result.error_code.val == MoveItErrorCodes.SUCCESS:
            ...
            self.node.get_logger().info(
                "MoveGroup planning 성공: "
                f"planner={request.planner_id or '<default>'}, "
                f"points={point_count}"
            )
            return True, result.planned_trajectory

        self.node.get_logger().error(
            "MoveGroup planning 실패: "
            f"error_code={result.error_code.val}, "
            f"planner={request.planner_id or '<default>'}, "
            f"goals={len(request.goal_constraints)}"
        )
        return False, None
    ...
    def _call_seeded_ik_once(
        ...
        joint_values = self._extract_ik_joint_values(response)
        ...
        if joint_list is not None:
            ...
            self.node.get_logger().info(f"{seed_label} IK 성공")
        return joint_values
    ...
    def rank_seeded_ik_candidates(self, pose_stamped, avoid_collisions=True):
        ...
        if not self.enable_multi_seed_ik:
            ...
            self.node.get_logger().info(
                "IK 후보 1개 선택: "
                f"current, cost={first_cost:.3f}, "
                f"q={self._fmt_joint_degrees(first_list)}"
            )
        ...
        self.node.get_logger().info(f"IK 후보 {len(candidates)}개 선택")
        for cost, _, joint_list, seed_label in candidates:
            self.node.get_logger().info(
                f"  후보 {seed_label}: "
                f"cost={cost:.3f}, q={self._fmt_joint_degrees(joint_list)}"
            )
        return candidates

    def solve_seeded_ik(self, pose_stamped, avoid_collisions=True):
    ...
    def _plan_safe_joint_candidate(
        ...
        for attempt, candidate in enumerate(
            ...
            self.node.get_logger().info(
                ...
                f"planning 시작 - {seed_label}, cost={cost:.3f}, "
                f"q={self._fmt_joint_degrees(joint_list)}"
            )
            ...
            self.node.get_logger().info(
                f"{label}: trajectory 후보 planning 성공 - {seed_label}"
            )
    ...
    def go_smooth(self, pose_stamped, label="pose"):
        ...
        if not candidates:
            self.node.get_logger().error(f"{label}: IK 후보를 찾지 못했습니다")
            return False
        ...
        if not plan_success or trajectory is None:
            self.node.get_logger().error(
                f"{label}: 안전한 IK trajectory 후보를 찾지 못했습니다"
            )
            return False
